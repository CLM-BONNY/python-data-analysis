...
# 한글폰트와 마이너스 폰트 설정 확인
pd.Series([-1, 0, 3, 5]).plot(title="한글폰트")
...
for i in df_cafe.index:
    ...
    folium.CircleMarker([sub_lat, sub_long], redius=3, color=color, tooltip=title).add_to(m)
...
geo_path = 'C:/Users/alstj/상가상권정보/seoul_municipalities_geo_simple.json'
...
for i in df_vs.index:
    sub_lat = df_vs.loc[i, "경도"]
    sub_long = df_vs.loc[i, "위도"]
